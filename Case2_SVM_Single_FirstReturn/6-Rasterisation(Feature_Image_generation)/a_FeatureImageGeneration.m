...
clear
close all
load featuretable_FirstReturn
% feature=featuretable_OneScale;
feature=(featuretable_FirstReturn-min(featuretable_FirstReturn))./(max(featuretable_FirstReturn)-min(featuretable_FirstReturn));
load VegPoints2
feature(:,5)=VegPoints2(:,11);  %%%this is number of segments

[image,RO1]=geotiffread('2013_SJER_AOP_Camera_sample.tif');
FeatureMAP=zeros(1000,1000,5);
...
ss=size(pr);
 for i=1:ss(1)
    ...
    FeatureMAP(pr(i,2),pr(i,1),:)=feature(i,:);
     elseif FeatureMAP(pr(i,2),pr(i,1),1)~=0 && feature(i,4)>FeatureMAP(pr(i,2),pr(i,1),4)
         FeatureMAP(pr(i,2),pr(i,1),:)=feature(i,:);
